...
```{r, echo=TRUE}
df <- read.csv("D:/2_Master/Q1/MA/mva_project/data_mva.csv")
...
ggplot(df, aes(Education, ..count..)) + geom_bar(aes(fill = EducationField), position = "dodge")
...
# Create new DataFrame to identify which are outliers
tmpDF<-df
...
sapply(nonFactorDf,min)
sapply(nonFactorDf[mdi>cutoff,],min)
...
#tt<-nonFactorDf[(mdi>cutoff),];tt
#tt<-tt[tt$MonthlyIncome<8000,];tt
```
